...
-- and end sequence. For example, consider:
--      <x | x > 2          and             <x | x > 2 >
...
-- token is a open sequence token, adds one to the current top of the sequence 
-- stack. Then, whenever a > token is discovered it checks to see if the top
-- value is non-zero, and if so lexes endseq, otherwise lexes gt. It then
-- decrements the value of the top of the sequence stack in the parser. It uses
-- a stack to allow it to open new contexts within parenthesis.
--
...
-- the decision about LT being openseq or < in the parser as this can decide
-- when this is the case (e.g. x < means it must be a LT). 
--
...
-- lookahead before we actually parse <, meaning that when we push the 1 onto
-- the sequence stack it actually goes onto the new top entry that is popped
-- off after ")" is processed. Therefore, the > will be parsed as a gt. To
-- solve this we make sure that whenever we pop off the sequence stack we add
-- any remaining open sequences onto the new top element. Clearly this would
-- be unsafe if we were relying on this for parsing, but as we are not this
-- should be fine and should cause no further ambiguities.
--
