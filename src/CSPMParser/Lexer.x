{
module CSPMParser.Lexer where

import Data.List

import CSPMDataStructures.Tokens
import CSPMParser.Monad
...
--	<0>"--".*					{ skip }
	...
	<0>@nl"[T="					{ tok TTraceRefines }
	<0>@nl"[F="					{ tok TFailuresRefines }
	<0>@nl"[FD="				{ tok TFailuresDivergencesRefines }
...
getNextToken = do
	...
	case alexScan st sc of
		...
		AlexError st' -> throwError $ LexicalError fname pos
		AlexSkip st' len -> do
			setParserState st'
			getNextToken
