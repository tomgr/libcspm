module CSPMParser.Monad
	(module Control.Monad.Error,
	ParserError(..), ParseMonad, ParserState(..), FilePosition(..), 
...
import CSPMDataStructures.Tokens
import Util.Monad
...
-- *************************************************************************
data ParserError = 
	LexicalError FilePath FilePosition
	| ParseError LToken
	| UnknownError String
	deriving Show
...
type ParseMonad = 
	ErrorT ParserError (StateT ParserState Tyger)

runParser :: ParseMonad a -> String -> Tyger a
runParser prog dirname =
	do
		(errOrVal, state) <-
			runStateT (runErrorT prog) (ParserState dirname [])
		case errOrVal of
